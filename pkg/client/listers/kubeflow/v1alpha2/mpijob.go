...
type MPIJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.MPIJob, err error)
...
// List lists all MPIJobs in the indexer.
func (s *mPIJobLister) List(selector labels.Selector) (ret []*v1alpha2.MPIJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.MPIJob))
	})
	return ret, err
}
...
type MPIJobNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.MPIJob, error)
...
// List lists all MPIJobs in the indexer for a given namespace.
func (s mPIJobNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.MPIJob, err error) {
...
// Get retrieves the MPIJob from the indexer for a given namespace and name.
func (s mPIJobNamespaceLister) Get(name string) (*v1alpha2.MPIJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("mpijob"), name)
	}
	return obj.(*v1alpha2.MPIJob), nil
}
